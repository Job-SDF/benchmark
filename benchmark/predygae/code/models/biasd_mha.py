...
class BiasedMHA(nn.Module):
    ...
    def forward(self, query, key, value, attn_bias=None, attn_mask=None):
        ...
        """
        q_h = self.q_proj(query).transpose(0, 1)
        k_h = self.k_proj(key).transpose(0, 1)
        v_h = self.v_proj(value).transpose(0, 1)
        bsz, q_n, _ = query.shape
        bsz, k_n, _ = key.shape
        bsz, v_n, _ = value.shape
        q_h = (
            q_h.reshape(q_n, bsz * self.num_heads, self.head_dim).transpose(0, 1)
            * self.scaling
        )
        k_h = k_h.reshape(k_n, bsz * self.num_heads, self.head_dim).permute(
            1, 2, 0
        )
        v_h = v_h.reshape(v_n, bsz * self.num_heads, self.head_dim).transpose(
            0, 1
        )
        ...
        attn_weights = (
            th.bmm(q_h, k_h)
            .transpose(0, 2)
            .reshape(k_n, q_n, bsz, self.num_heads)
            .transpose(0, 2)
        )
        ...
        attn_weights = F.softmax(attn_weights.transpose(0, 2).reshape(k_n, q_n, bsz * self.num_heads).transpose(0, 2),dim=2,)
        ...
        attn = self.out_proj(
            attn.reshape(q_n, bsz, self.feat_size).transpose(0, 1)
        )
